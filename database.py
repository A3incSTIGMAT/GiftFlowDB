...
def get_db_cursor(commit: bool = True):
    ...
    Args:
        commit: Если False, не делать автоматический коммит (для ручного управления)
    """
...
def get_db_connection():
    ...
    conn = sqlite3.connect(
        str(DB_PATH),
        timeout=10.0,  # Ждать 10 сек при блокировке БД
    ...
    conn.execute("PRAGMA journal_mode = WAL")  # Лучшая производительность
    conn.execute("PRAGMA synchronous = NORMAL")  # Баланс скорости и надёжности
    conn.execute("PRAGMA cache_size = -64000")  # Кэш 64MB
    return conn
...
def register_user_sync(user_id: int, username: str = None, first_name: str = None, last_name: str = None):
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            logger.debug(f"✅ Пользователь {user_id} зарегистрирован/обновлён")
    except sqlite3.Error as e:
...
def add_gift_sync(name: str, price: int, description: str = "", icon: str = "🎁", is_active: int = 1) -> int:
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            logger.info(f"✅ Подарок добавлен: {name} (ID: {gift_id})")
            return gift_id
    except sqlite3.Error as e:
...
def update_gift_sync(gift_id: int, name: str = None, price: int = None, 
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            cursor.execute(f"""
                UPDATE gifts SET {', '.join(updates)} WHERE id = ?
            """, params)
            
            updated = cursor.rowcount > 0
            if updated:
                logger.info(f"✅ Подарок #{gift_id} обновлён")
            return updated
    except sqlite3.Error as e:
...
def create_order_sync(user_id: int, gift_id: int, amount: int, username: str = None) -> int:
    ...
    if not isinstance(user_id, int) or user_id <= 0:
        raise ValueError(f"Некорректный user_id: {user_id}")
    if not isinstance(gift_id, int) or gift_id <= 0:
        raise ValueError(f"Некорректный gift_id: {gift_id}")
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            logger.info(f"✅ Заказ #{order_id} создан: пользователь {user_id}, сумма {amount}")
            return order_id
    except sqlite3.Error as e:
...
def confirm_order_sync(order_id: int, confirmed_by: int = None) -> bool:
    """Подтвердить заказ и обновить топ героев"""
    if not isinstance(order_id, int) or order_id <= 0:
        return False
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            if cursor.rowcount > 0:
                ...
                username = order['real_username'] or order['username']
                update_top_heroes_sync(order['user_id'], order['amount'], username)
                logger.info(f"✅ Заказ #{order_id} подтверждён, сумма {order['amount']}")
                return True
...
def add_transaction_sync(user_id: int, gift_id: int, amount: int, 
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            logger.info(f"✅ Транзакция #{transaction_id} создана: пользователь {user_id}, сумма {amount}")
...
def update_transaction_status_sync(transaction_id: int, status: str, confirmed_by: int = None) -> bool:
    ...
    try:
        with get_db_cursor() as cursor:
            # Получаем данные транзакции ДО обновления (с именем пользователя из users)
            cursor.execute("""
            ...
            if not transaction:
                logger.warning(f"⚠️ Транзакция #{transaction_id} не найдена или уже обработана")
                return False
            ...
            if cursor.rowcount > 0 and status == 'paid':
                # Обновляем топ героев только при успешной оплате
                update_top_heroes_sync(
                    transaction['user_id'], 
                    transaction['amount'], 
                    transaction['username']
                )
                logger.info(f"✅ Транзакция #{transaction_id} оплачена, сумма {transaction['amount']}")
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            if row:
                ...
                cursor.execute("""
                    UPDATE top_heroes 
                    SET total_amount = ?, last_donate = CURRENT_TIMESTAMP, 
            ...
            logger.debug(f"✅ Топ героев обновлён: пользователь {user_id} +{amount}")
    except sqlite3.Error as e:
...
def add_gallery_photo_sync(file_id: str, description: str = "", added_by: int = None) -> int:
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            logger.info(f"✅ Фото добавлено в галерею (ID: {photo_id})")
            return photo_id
    except sqlite3.Error as e:
...
def delete_gallery_photo_sync(photo_id: int) -> bool:
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            if deleted:
                logger.info(f"✅ Фото #{photo_id} удалено из галереи")
            return deleted
    except sqlite3.Error as e:
...
def is_admin_sync(user_id: int) -> bool:
    ...
    if user_id in (SUPER_ADMIN_ID, SUPPORT_ADMIN_ID):
        return True
    ...
    try:
        with get_db_cursor() as cursor:
            cursor.execute("""
                ...
                VALUES (?, ?, ?)
            """, (user_id, None, added_by))
            added = cursor.rowcount > 0
            if added:
                logger.info(f"✅ Пользователь {user_id} добавлен в админы")
            return added
    except sqlite3.Error as e:
...
def remove_admin_sync(user_id: int) -> bool:
    ...
    if user_id in (SUPER_ADMIN_ID, SUPPORT_ADMIN_ID):
        logger.warning(f"⚠️ Нельзя удалить супер-админа {user_id}")
        return False
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            if removed:
                logger.info(f"✅ Пользователь {user_id} удалён из админов")
            return removed
    except sqlite3.Error as e:
...
def get_statistics_sync() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "total_users": 0, "total_orders": 0, "total_amount": 0,
            "total_pending": 0, "total_donations": 0, "month_amount": 0
        }
...
def get_goal_progress_sync() -> dict:
    ...
    if goal_amount > 0:
        percent = min(int(collected / goal_amount * 100), 100)
    else:
        percent = 0
...
def update_goal_sync(goal_name: str = None, goal_amount: int = None) -> bool:
    ...
    if goal_amount is not None and (not isinstance(goal_amount, int) or goal_amount < 0):
        raise ValueError("goal_amount должен быть неотрицательным целым числом")
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            if updates:
                ...
                """, params)
                logger.info(f"✅ Цель обновлена: {goal_name or '...'} ({goal_amount or '...'})")
                return True
            return False
    except sqlite3.Error as e:
...
async def init_db():
    ...
    return await asyncio.to_thread(init_database)


async def register_user(user_id: int, username: str = None, first_name: str = None, last_name: str = None):
    return await asyncio.to_thread(register_user_sync, user_id, username, first_name, last_name)
...
async def get_all_gifts(active_only: bool = True):
    return await asyncio.to_thread(get_all_gifts_sync, active_only)


async def get_gift_by_id(gift_id: int):
    return await asyncio.to_thread(get_gift_by_id_sync, gift_id)


async def add_gift(name: str, price: int, description: str = "", icon: str = "🎁", is_active: int = 1):
    return await asyncio.to_thread(add_gift_sync, name, price, description, icon, is_active)


async def update_gift(gift_id: int, name: str = None, price: int = None, 
                      description: str = None, icon: str = None, is_active: int = None):
    return await asyncio.to_thread(update_gift_sync, gift_id, name, price, description, icon, is_active)


async def create_order(user_id: int, gift_id: int, amount: int, username: str = None):
    return await asyncio.to_thread(create_order_sync, user_id, gift_id, amount, username)


async def get_pending_orders(limit: int = 100):
    return await asyncio.to_thread(get_pending_orders_sync, limit)


async def get_order_by_id(order_id: int):
    return await asyncio.to_thread(get_order_by_id_sync, order_id)


async def confirm_order(order_id: int, confirmed_by: int = None):
    return await asyncio.to_thread(confirm_order_sync, order_id, confirmed_by)


async def cancel_order(order_id: int):
    return await asyncio.to_thread(cancel_order_sync, order_id)


async def add_transaction(user_id: int, gift_id: int, amount: int, 
                          payment_method: str = None, order_id: int = None):
    return await asyncio.to_thread(add_transaction_sync, user_id, gift_id, amount, payment_method, order_id)


async def update_transaction_status(transaction_id: int, status: str, confirmed_by: int = None):
    return await asyncio.to_thread(update_transaction_status_sync, transaction_id, status, confirmed_by)


async def get_pending_transactions(limit: int = 50):
    return await asyncio.to_thread(get_pending_transactions_sync, limit)


async def get_all_transactions(limit: int = 100, status: str = None):
    return await asyncio.to_thread(get_all_transactions_sync, limit, status)


async def update_top_heroes(user_id: int, amount: int, username: str = None):
    return await asyncio.to_thread(update_top_heroes_sync, user_id, amount, username)


async def get_top_heroes(limit: int = 10):
    return await asyncio.to_thread(get_top_heroes_sync, limit)


async def add_gallery_photo(file_id: str, description: str = "", added_by: int = None):
    return await asyncio.to_thread(add_gallery_photo_sync, file_id, description, added_by)


async def get_gallery_photos(limit: int = 50):
    return await asyncio.to_thread(get_gallery_photos_sync, limit)


async def delete_gallery_photo(photo_id: int):
    return await asyncio.to_thread(delete_gallery_photo_sync, photo_id)


async def is_admin(user_id: int):
    return await asyncio.to_thread(is_admin_sync, user_id)


async def is_super_admin(user_id: int):
    return await asyncio.to_thread(is_super_admin_sync, user_id)


async def add_admin(user_id: int, added_by: int = None):
    return await asyncio.to_thread(add_admin_sync, user_id, added_by)
...
async def log_admin_action(admin_id: int, action: str, details: str = None):
    return await asyncio.to_thread(log_admin_action_sync, admin_id, action, details)


async def get_statistics():
    return await asyncio.to_thread(get_statistics_sync)
...
async def get_goal_progress():
    return await asyncio.to_thread(get_goal_progress_sync)


async def update_goal(goal_name: str = None, goal_amount: int = None):
    return await asyncio.to_thread(update_goal_sync, goal_name, goal_amount)
