...
from config import SUPER_ADMIN_ID, SUPPORT_ADMIN_ID, CHANNEL_ID
...
async def pay_by_card(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = f"{gift['icon']} <b>{gift['name']}</b>\n\n💰 Сумма: <b>{gift['price']}₽</b>\n\n{PAYMENT_DETAILS['card']}\n\n⚠️ <i>Укажите в комментарии Telegram ID: <code>{callback.from_user.id}</code></i>"
...
async def confirm_payment(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"✅ <b>Спасибо за поддержку!</b>\n\nЗаказ #{transaction_id} принят.\n🎁 {gift_name} — {gift_price}₽\n\n⏳ Менеджер проверит оплату.\n💰 После подтверждения вы попадёте в топ героев!",
        parse_mode="HTML",
